...
$ob = new connect();
if(isset($_GET['p']) && is_numeric($_GET['p']) && isset($_GET['wid'])){
    $page = $_GET['p'];
    $wid = $_GET['wid'];
    if(empty($wid)){
        header("location:not_found.html");
    }
    $record_count = $ob->select_data2("select * from posts where wid=$wid");
    $per_page = 10;
...
$next = $page+1;
if(isset($_GET['wid'])){
    $timeline = $ob->select_data2("select post_id,title, body,category, img, categories.category_id ,posted from posts INNER JOIN categories on categories.category_id=posts.category_id where posts.wid='$wid' order by post_id desc limit $start,$per_page");
}else{
header("location:blog.php");
}
?>
        ...
        <div class="row">
            ...
            <div class="col-md-8">
                ...
                <ul style="visibility:hidden;" class="pager">             
                  ...
                  if($timeline->num_rows>0){    
                    if($prev>0){
                        echo "<a href='writers_post.php?p=$prev&wid=$wid'><span class='glyphicon glyphicon-arrow-left'></span>&nbsp;Previous</a>";
                    }
                  }
                ?>
                    </li>
                    <li class="next">
                       <?php
                          if($page<$pages)  {
                              echo "<a href='writers_post.php?p=$next&wid=$wid'>Next&nbsp;&nbsp;&nbsp;<span class='glyphicon glyphicon-arrow-right'></span></a>";
                          }        
                                ?>
