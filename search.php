...
if(isset($_GET['search'])){
    ...
    $record_count = $ob->select_data2("select * from posts where title = '$search'");
    $pages = ceil($record_count->num_rows/$per_page);
    $timeline = $ob->select_data2("select post_id,title, body,category, img, categories.category_id , posted from posts INNER JOIN categories on categories.category_id=posts.category_id where posts.title = '$search' order by post_id desc limit $start,$per_page");
}else if(isset($_GET['searchName'])){
    $search = $_GET['searchName'];
    $record_count = $ob->select_data2("select * from posts where title = '$search'");
    $pages = ceil($record_count->num_rows/$per_page);
    $timeline = $ob->select_data2("select post_id,title,LEFT(body,200) AS body,category, img, categories.category_id from posts INNER JOIN categories on categories.category_id=posts.category_id where posts.title like '$search' order by post_id desc limit $start,$per_page");
}else{
        ...
        <div class="row">
            ...
            <div class="col-md-8">
                ...
                <ul style="visibility:hidden;" class="pager">             
                  ...
                  if($timeline->num_rows>0){    
                    if($prev>0){
                        echo "<a href='index.php?p=$prev'><span class='glyphicon glyphicon-arrow-left'></span>&nbsp;Previous</a>";
                    }
                  }
                ?>
                    </li>
                    <li class="next">
                       <?php
                          if($page<$pages)  {
                              echo "<a href='index.php?p=$next'>Next&nbsp;&nbsp;&nbsp;<span class='glyphicon glyphicon-arrow-right'></span></a>";
                          }        
                                ?>
